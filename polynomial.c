...
static void *polynomial_get_coef_ptr(const Polynomial* poly, int index)
{
    ...
    if(index < 0 || (size_t)index >= poly->capacity) return NULL;
...
static void polynomial_update_degree(Polynomial* poly)
{
    ...
    for(int i = poly->degree; i > 0; i--)
    {
        ...
        if(poly->type_info->is_zero) zero = poly->type_info->is_zero(coef);
        else zero = 0;
...
int polynomial_add(Polynomial* result, const Polynomial* a, const Polynomial* b)
{
    ...
    if(!result->type_info->add || !result->type_info->zero) return -1;

    max_degree = (a->degree > b->degree) ? (int)a->degree : (int)b->degree;

    if(polynomial_capacity(result, max_degree + 1) != 0) return -1;

    for(int i = 0; i <= max_degree; i++)
    {
        coef_a = (i <= (int)a->degree) ? polynomial_get_coef_ptr(a, i) : NULL;
        coef_b = (i <= (int)b->degree) ? polynomial_get_coef_ptr(b, i) : NULL;
        ...
        if(coef_a && coef_b)
        {
            result->type_info->add(coef_a, coef_b, coef_result);
        }
        else if(coef_a)
        {
            memcpy(coef_result, coef_a, result->type_info->size_el);
        }
        else if(coef_b)
        {
            memcpy(coef_result, coef_b, result->type_info->size_el);
        }
        else
        {
            result->type_info->zero(coef_result);
        }
    }
...
{
    int result_degree;
    void *coef_a, *coef_b, *coef_result;
    void *temp;

    if (!result || !a || !b) return -1;
    if (!field_info_compare(a->type_info, b->type_info)) return -1;
    if (!result->type_info->add || !result->type_info->mult) return -1;

    result_degree = (int)(a->degree + b->degree);
    ...
    for (int i = 0; i <= a->degree; i++)
    {
        ...
        {
            coef_a = polynomial_get_coef_ptr(a, i);
            coef_b = polynomial_get_coef_ptr(b, j);
            coef_result = polynomial_get_coef_ptr(result, i + j);

            if (!coef_a || !coef_b || !coef_result) continue;

            temp = malloc(result->type_info->size_el);
            if (!temp) return -1;

            result->type_info->mult(coef_a, coef_b, temp);
            result->type_info->add(coef_result, temp, coef_result);
...
int polynomial_mult_scal(Polynomial* poly, const void* scalar)
{
    ...
    if (!poly || !scalar || !poly->type_info->mult_scal) return -1;
    ...
    for (int i = 0; i <= poly->degree; i++)
    {
        ...
        if (coef)
        {
            poly->type_info->mult_scal(coef, scalar);
        }
    }
...
void polynomial_print(const Polynomial* poly)
{
    ...
    for (int i = (int)poly->degree; i >= 0; i--)
    {
        ...
        if (poly->type_info->is_zero && poly->type_info->is_zero(coef) && i > 0)
            continue;
